...
final readonly class ClotheWorkflowService
{
     ...
     */
    public function getEnabledTransitions(ClothesVariant $variant): array
    {
        return $this->clothePublicationStateMachine
    ...
    public function reconcilePublicationEligibilityWithoutFlush(ClothesVariant $variant): bool
    {
        ...
        foreach ($transitions as $transition) {
            if (!$this->clothePublicationStateMachine->can($variant, $transition)) {
                throw new \DomainException(sprintf('La transition automatique « %s » est impossible depuis l’état %s.', $transition, $variant->getPublicationStatus()->label()));
            }
