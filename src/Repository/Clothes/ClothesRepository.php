...
class ClothesRepository extends ServiceEntityRepository
{
    ...
    public function findOneByNameOrSlugExcludingSlug(
        string $name,
        string $slug,
        ?string $excludedSlug = null,
    ): ?Clothes {
        $qb = $this->createQueryBuilder('c');
        ...
        if (null !== $excludedSlug && '' !== $excludedSlug) {
            $qb
                ->andWhere(sprintf(
                    'c.id NOT IN (SELECT excludedClothe.id FROM %s excludedClothe '
                    . 'JOIN excludedClothe.variants excludedVariant '
                    . 'WHERE excludedVariant.slug = :excludedSlug)',
                    Clothes::class,
                ))
    ...
    public function findClothesInCollection($collection): array
    {
        $qb = $this->createQueryBuilder('c');
        $qb->select(
            'DISTINCT c.name, variants.images, variants.publicationStatus, '
            . 'co.name As collectionName, cc.name AS colorName, c.createdAt',
        )
     ...
     * @return Clothes[] Returns an array of Clothes objects
     */
    public function findDistinctBySlug(
        ?string $orderBy,
        ?string $direction,
        ?string $query = null,
        ?int $limit = null,
    ...
    public function findDistinctBestsellerEntities(?int $limit = null): array
    {
        ...
        return null !== $limit ? array_slice($results, 0, $limit) : $results;
    }
    ...
    public function findDistinctFeaturedEntities(): array
    {
        ...
        foreach ($results as $clothe) {
            if ($clothe instanceof Clothes && null !== $clothe->getSlug()) {
