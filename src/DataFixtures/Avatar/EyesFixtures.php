...
final class EyesFixtures extends AbstractBaseFixtures implements DependentFixtureInterface
{
    ...
    {
        foreach (['blue', 'green', 'brown'] as $colorIndex => $colorName) {
            foreach (['round', 'almond'] as $shapeIndex => $shapeName) {
                $name = sprintf('eye__%s__%s', $colorName, $shapeName);

                $eye = (new Eye())
                    ->setName($name)
                    ->setColor($this->getReference(FixtureReferences::EYE_COLORS.$colorIndex, \App\Entity\Avatar\Eyes\Eyecolor::class))
                    ->setShape($this->getReference(FixtureReferences::EYE_SHAPES.$shapeIndex, \App\Entity\Avatar\Eyes\Eyeshape::class))
